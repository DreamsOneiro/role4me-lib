...
impl Character {
    /// Create empty 5th edition character
    ///
    /// Example:
    /// ```
    /// use role4me_lib::Character;
    ///
    /// let c = Character::new_5e();
    /// ````
    ...
    /// Select race for the character
    ///
    /// Use Race(SubRace::Sub) to select race
    /// e.g. Human(SubHuman::Basic), or Elf(SubElf::High) 
    ///
    /// Return true if succesfully changed race
    ///
    /// Example:
    /// ```
    /// c.select_race_5e(Human(SubHuman::Basic));
    /// ```
    pub fn select_race_5e(&mut self, new_race: Race) -> bool {
        check_edition_5e!(self);
        // Clear used AP
        if self.race != new_race {
            self.race = new_race;
    ...
    /// Use ability point(s) from race
    /// Use case: 
    /// Variant human can select 2
    /// ability to increase by 1 point
    ///
    /// Accpeted strings are:
    ...
    /// Example:
    /// ```
    /// let c = Character::new_character_5e();
    /// c.select_race_5e(Human(SubHuman::Variant));
    /// c.use_race_ap_5e("str"); // return true
    /// c.use_race_ap_5e("str"); // return false
    /// c.use_race_ap_5e("int"); // return true
    /// c.use_race_ap_5e("dex"); // return false
    /// ```
    ///
    /// Refer to D&D rules to understand the bool
    /// return value
    ...
    /// Remove one race ability point
    ///
    /// Accpeted strings are:
    /// "str", "dex", "con", "int", "wis", "cha"
    ///
    /// Return true only if successfully removed point
    ///
    /// Example:
    /// ```
    /// c.remove_race_ap_5e("str");
    /// ```
    ...
    /// Clear all added race ability points
    /// 
    /// Return true even if character has no
    /// used ability but return false if failed
    /// to clear ability
    ...
    /// Add language to profeciency
    ///
    /// Example strings:
    /// "Common", "Elven", "Dwarven"
    ///
    /// Will not check for spelling
    /// e.g. Dwarven and Dwarvish will be treated
    ///      as diffent langauge
    ///
    /// Will not check for casing
    /// e.g. Dwarven and dwarven will be treated
    ...
    /// Remove manually added language
    /// Can't remove default language assigned by race
    ///
    /// Return true only if successfully removed point
    ///
    /// Same rule for string as [use_lang_point] function
    ///
    /// Example:
    /// ```
    /// c.remove_lang_5e("Dwarven");
    /// ```
    ...
    /// Clear all manually added language
    ///
    /// Return true even if character has no
    /// used language but return false if failed
    /// to clear language list
    ...
    /// Use "roll" method to assign base ability scores
    ///
    /// Use [assign_rolls_5e] function to assign
    /// the sequence for each ability. 
    ///
    /// Rolled values are saved in c.unassigned_base_ap
    ...
    /// Use standard array to assign base ability scores
    ///
    /// Use [assign_rolls_5e] function to assign
    /// the sequence for each ability. 
    ///
    /// Array values are saved in c.unassigned_base_ap
    pub fn standard_array_5e(&mut self) -> bool {
        check_edition_5e!(self);
        self.unassigned_base_ap = [15,14,13,12,10,8];
        true
    }

    /// Use heroic array to assign base ability scores
    ///
    /// Use [assign_rolls_5e] function to assign
    /// the sequence for each ability. 
    ///
    /// Array values are saved in c.unassigned_base_ap
    pub fn heroic_array_5e(&mut self) -> bool {
        check_edition_5e!(self);
        self.unassigned_base_ap = [17,16,15,14,12,10];
        true
    }

    /// Use 'point buy' method to assign base 
    /// ability points
    ///
    /// Pass in array for points, valued between 0 to 9
    /// 6 & 8 are not allowed (refer to D&D rule book)
    /// Sum of the value must not exceed 27
    /// i.e. [1,5,7,9,3,2] 
    ///
    ...
    /// Example:
    /// ```
    /// c.point_buy_5e([1,5,7,9,3,2]);
    /// ```
    pub fn point_buy_5e(&mut self, points: [u8; 6]) -> bool {
    ...
    /// 3) Heroic Array
    ///
    /// Check values in c.unassigned_base_ap variable
    ///
    /// Pass in sequence to assign ap from 1 to 6
    /// i.e [2,5,1,3,4,6]
    ///
    /// Function will not accept overlap sequence
    ///
    /// Return true if successfully assigned sequence
    ///
    /// Example:
    /// ```
    /// // Case 1
    /// c.assign_rolls_5e([2,5,1,3,4,6]); // true
    /// // Case 2
    /// c.assign_rolls_5e([1,1,1,1,1,1]); // false
    /// ```
    ...
    /// Initialize points and add them to
    /// total ability scores
    ///
    /// Use after assigning base points
